...
//   )
// }
// 📄 TraineeDashboard.jsx – Enhanced with Class and Competition Sections
// 📄 TraineeDashboard.jsx – Enhanced with Class and Competition Sections
import { useEffect, useState } from 'react'
import { useUserStore } from '../store/userStore'
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500">{t('manageClasses.title')}</p>
          </div>
          <button onClick={logout} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-gray-200 hover:border-red-200 hover:bg-red-50 text-red-600 transition-all duration-300 group">
            <span className="font-medium">Logout</span>
            <ArrowRightIcon className="w-4 h-4 transition-transform group-hover:translate-x-0.5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Link to="/trainee/classes">
            <div className="bg-white hover:bg-teal-50 p-6 rounded-xl border border-teal-200 shadow-sm cursor-pointer transition">
              <AcademicCapIcon className="w-8 h-8 text-teal-600 mb-2" />
              <h2 className="text-xl font-bold text-gray-800 mb-2">{t('manageClasses.enrollTitle')}</h2>
              <p className="text-gray-600 text-sm">{t('manageClasses.enrollDescription')}</p>
            </div>
          </Link>

          <Link to="/trainee/competitions">
            <div className="bg-white hover:bg-yellow-100 p-6 rounded-xl border border-yellow-300 shadow-sm cursor-pointer transition">
              <TrophyIcon className="w-8 h-8 text-yellow-500 mb-2" />
              <h2 className="text-xl font-bold text-gray-800 mb-2">{t('manageCompetitions.title')}</h2>
              <p className="text-sm text-gray-600">{t('manageCompetitions.description')}</p>
            </div>
          </Link>
        </div>
      </div>
